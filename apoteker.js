      ...
      <div class="flex h-screen overflow-hidden w-full">
        ...
        <main class="flex-1 flex flex-col md:ml-64 pt-14 md:pt-0 overflow-y-auto overflow-x-hidden min-w-0 relative">
          ...
          <div class="px-4 sm:px-8 lg:px-10 py-6 space-y-6 max-w-full">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                <div class="flex items-center gap-3 mb-2">
                  <div class="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center">
                    <i data-lucide="package-minus" class="w-5 h-5 text-emerald-500"></i>
                  </div>
                  <div>
                    <p class="text-xs text-slate-400">Hari Ini</p>
                    <p class="text-xl font-bold text-slate-800">${todayOutflow.length} Transaksi</p>
                  </div>
                </div>
              </div>
              <div class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                <div class="flex items-center gap-3 mb-2">
                  <div class="w-10 h-10 rounded-xl bg-teal-50 flex items-center justify-center">
                    <i data-lucide="pill" class="w-5 h-5 text-teal-500"></i>
                  </div>
                  <div>
                    <p class="text-xs text-slate-400">Total Obat Keluar</p>
                    <p class="text-xl font-bold text-slate-800">${todayOutflow.reduce((sum, o) => sum + (o.amount || 0), 0)} Item</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- Add Outflow Form -->
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
              <h3 class="text-sm font-bold text-slate-900 mb-4">Catat Pengeluaran Obat</h3>
              <form id="apotekerOutflowForm" class="grid sm:grid-cols-4 gap-4">
                <select name="medId" required class="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500">
                  <option value="">Pilih Obat</option>
                  ${medicineStock.filter(m => m.status === 'Tersedia').map((m, i) => `<option value="${i}">${m.name} (Stok: ${m.qty || 0})</option>`).join('')}
                </select>
                <input type="number" name="amount" placeholder="Jumlah" min="1" required class="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500">
                <input type="text" name="patient" placeholder="Nama pasien (opsional)" class="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500">
                <button type="submit" class="px-4 py-2.5 bg-emerald-500 text-white rounded-xl text-sm font-semibold hover:bg-emerald-600 transition-all flex items-center justify-center gap-2">
                  <i data-lucide="save" class="w-4 h-4"></i>
                  <span>Catat</span>
                </button>
              </form>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div class="p-4 border-b border-slate-100 flex items-center justify-between">
                <h3 class="text-sm font-bold text-slate-900">Riwayat Pengeluaran</h3>
                <div class="flex items-center gap-2">
                  <button onclick="printOutflowHistory()" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-sky-50 text-sky-600 hover:bg-sky-100 transition-all text-xs font-semibold" title="Print Riwayat">
                    <i data-lucide="printer" class="w-3.5 h-3.5"></i>
                    <span class="hidden sm:inline">Print</span>
                  </button>
                  <button onclick="exportOutflowToExcel()" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-all text-xs font-semibold" title="Export to Excel">
                    <i data-lucide="file-spreadsheet" class="w-3.5 h-3.5"></i>
                    <span class="hidden sm:inline">Excel</span>
                  </button>
                </div>
              ...
              <div class="divide-y divide-slate-100">
                ${todayOutflow.length > 0 ? todayOutflow.map(o => `
                  <div class="p-4 flex items-center justify-between hover:bg-slate-50/50">
                    <div class="flex items-center gap-3">
                      <div class="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center">
                        <i data-lucide="arrow-up-from-line" class="w-5 h-5 text-indigo-500"></i>
                      </div>
                      <div>
                        <p class="text-sm font-semibold text-slate-800">${o.medicineName}</p>
                        <p class="text-xs text-slate-400">${o.patient || 'Tanpa nama'} • ${o.time}</p>
                      </div>
                    </div>
                    <span class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-xs font-semibold">${o.amount} item</span>
                  </div>
                `).join('') : `
                  <div class="p-8 text-center text-slate-400">
                    <i data-lucide="inbox" class="w-12 h-12 mx-auto mb-3 opacity-50"></i>
                    <p class="text-sm">Belum ada pengeluaran hari ini</p>
                  </div>
                `}
              </div>
            </div>
